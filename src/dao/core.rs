...
use crate::dao::proposals::VersionedProposal;
use crate::dao::upgrade::{ FactoryInfo, internal_get_factory_info, internal_set_factory_info };
use crate::dao::policy::VersionedPolicy;
use crate::dao::bounties::{ BountyClaim, VersionedBounty };
...
pub extern "C" fn store_blob() {
    ...
    let mut contract: Dao = env::state_read().expect("ERR_CONTRACT_IS_NOT_INITIALIZED");
    ...
    let blob_hash_str = near_sdk::serde_json
        ::to_string(&Base58CryptoHash::from(blob_hash))
        .unwrap()
        .into_bytes();
...
mod tests {
    ...
    use crate::dao::proposals::{ ProposalKind, ProposalInput, ProposalStatus };
    use crate::dao::types::{ OLD_BASE_TOKEN, Action };
    use crate::dao::policy::VersionedPolicy;

    fn create_proposal(context: &mut VMContextBuilder, contract: &mut Dao) -> u64 {
    ...
    fn test_basics() {
        ...
        testing_env!(
            context.predecessor_account_id(accounts(2)).attached_deposit(to_yocto("1")).build()
        );
    ...
    fn test_remove_proposal_allowed() {
        ...
        let mut contract = Dao::new(Config::test_config(), policy);
        let id = create_proposal(&mut context, &mut contract);
        assert_eq!(contract.get_proposal(id).proposal.description, "test");
        contract.act_proposal(id, Action::RemoveProposal, None);
