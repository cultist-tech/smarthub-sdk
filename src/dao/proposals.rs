...
use crate::*;
use crate::dao::policy::{ RolePermission, VotePolicy, Policy, UserInfo };
use crate::dao::upgrade::{
    FactoryInfo,
    upgrade_remote,
    upgrade_using_factory,
    internal_set_factory_info,
};
use crate::dao::bounties::{ Bounty, VersionedBounty };
use crate::dao::types::{
    OldAccountId,
    Action,
    ONE_YOCTO_NEAR,
...
impl Proposal {
    ...
    pub fn update_votes(
        ...
        for role in roles {
            let amount = if
                policy.is_token_weighted(role, &self.kind.to_policy_label().to_string())
            {
                user_weight
            } else {
                1
