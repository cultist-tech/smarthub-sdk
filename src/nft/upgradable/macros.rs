...
macro_rules! impl_non_fungible_token_upgradable {
    ($contract:ident, $tokens:ident, $assert_owner:ident $(, $assert_upgrade:ident)?) => {
        ...
        impl NonFungibleTokenUpgradable for $contract {
            ...
            fn nft_set_upgrade_price(&mut self, rarity: mfight_sdk::nft::TokenRarity,ft_token_id: near_sdk::AccountId, price: U128) {
                self.$assert_owner();
                self.$tokens.nft_set_upgrade_price(rarity,ft_token_id, price)
            }
