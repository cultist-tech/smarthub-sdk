...
macro_rules! impl_non_fungible_token_burner {
    ($contract:ident, $tokens:ident, $assert_owner:ident $(, $assert_upgrade:ident)?) => {
        ...
        impl NonFungibleTokenBurner for $contract {            
            ...
            fn nft_set_burner_upgrade_price(&mut self, types: Option<mfight_sdk::nft::TokenTypes>, rarity: mfight_sdk::nft::TokenRarity, burning_rarity_sum: u8) {
                self.$assert_owner();
                self.$tokens.nft_set_burner_upgrade_price(types, rarity, burning_rarity_sum)
            }
